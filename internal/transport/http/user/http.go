...
func NewServer(endpoints user.Endpoints, options []kithttp.ServerOption, cfg *config.Configuration, service userService.IService) http.Handler {
	...
	r.Post("/users", kithttp.NewServer(endpoints.Create, decodeCreateRequest, kithttp.EncodeJSONResponse, options...).ServeHTTP)
	...
	r.Get("/users/check-auth", kithttp.NewServer(endpoints.CheckAuth, decodeCheckAuthRequest, kithttp.EncodeJSONResponse, options...).ServeHTTP)
